...
def main(data_file, results_file, save_path):
    ...
    for idx, row in results_df.iterrows():
        if "nude" in row["exp"]: pass
        print(row["exp"])
        true_results = utils.exp_funct(xvals, row["a"], row["b"], row["c"])
        if "A1" in row["exp"]:
            run_results = utils.runNtimes_t_pure(n, row["initS"], row["initT"], row["avg_growS"], 
                                            row["est_intrST"], row["est_intrTS"])
        elif "A5" in row["exp"]:
            run_results = utils.runNtimes_t_pure(n, row["initR"], row["initT"], row["avg_growR"], 
                                            row["est_intrRT"], row["est_intrTR"])
        else:
            run_results = utils.runNtimes_t(n, row["initR"], row["initS"], row["initT"], row["avg_growR"], row["avg_growS"], 
                        row["avg_intrRS"], row["avg_intrSR"], row["est_intrRT"], 
...
if __name__ == "__main__":
    data_file = ""
    results_file = "results/interaction_results/without_growT/t_interaction.csv"
