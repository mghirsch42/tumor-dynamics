...
def update_results(curr_results, params, opt_results):
    ...
    curr_results["est_intrTS"].append(opt_results.x[3])
...
def one_run(exp_file, clone_intr_file, t_intr_pure_file, t_init_file, params):
    ...
    for idx, row in df.iterrows():
        ...
        opt_results = dual_annealing(utils.one_step_error_t,
                        x0 = [params["init_intrRT"], params["init_intrST"],
...
def main(exp_file, clone_intr_file, t_intr_pure_file, t_init_file, save_file):
    ...
    for i in range(50):
        ...
        params = {
            ...
            "init_intrTR": np.random.uniform(param_bounds["init_intrTR"][0], param_bounds["init_intrTR"][1]),
            "init_intrTS": np.random.uniform(param_bounds["init_intrTS"][0], param_bounds["init_intrTS"][1]),
            "bounds": opt_bounds
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--clone_intr_file", action="store", default="results/method3/clone_interaction.csv")
    parser.add_argument("--t_intr_pure_file", action="store", default="results/method3/t_interaction_pure.csv")
    parser.add_argument("--t_init_file", action="store", default="data/t_cell_starting_sizes.csv")
    parser.add_argument("--save_file", action="store", default="results/method3/t_interaction.csv")
