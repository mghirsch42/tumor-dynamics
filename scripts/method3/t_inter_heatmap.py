...
rt_df = df[["exp", "id", "est_intrRT", "est_intrTR"]]
st_df = df[["exp", "id", "est_intrST", "est_intrTS"]]
...
# exps = rt_avg["exp"].unique()
# cbar = False
# for i in range(len(exps)):
#     exp_data = rt_avg[rt_avg["exp"]==exps[i]]
#     if i == len(exps)-1:
#         cbar = True
...
# fig, axes = plt.subplots(nrows=1, ncols=4)
# exps = st_avg["exp"].unique()
# cbar = False
# for i in range(len(exps)):
#     exp_data = st_avg[st_avg["exp"]==exps[i]]
#     if i == len(exps)-1:
#         cbar = True
