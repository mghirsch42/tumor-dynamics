...
def rep_aggs(est_df):
    est_df = est_df[["exp", "id", "est_growVal"]]
    est_df = est_df[est_df["est_growVal"] < .9]
...
def main(data_file, exp_file, est_file, save_path, save, show):
    ...
    for idx, row in exp_df.iterrows():
        ...
        y_true = [exp_funct(x, 1, row["b"], 0) for x in xvals]
        medx = curr_est[curr_est["est_growVal"] < 0.9]["est_growVal"].median()
        minx = curr_est[curr_est["est_growVal"] < 0.9]["est_growVal"].min()
        maxx = curr_est[curr_est["est_growVal"] < 0.9]["est_growVal"].max()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp_file", type=str, action="store", default="results/growth_fit_results_3/exp_growth.csv")
    parser.add_argument("--est_file", type=str, action="store", default="results/method9/clone_growth_rates.csv")
    parser.add_argument("--save_path", type=str, action="store", default="figures/method9/eqs_curves/")
