...
def runNtimes_t(n, initR, initS, initT, growR, growS, intrRS, intrSR, intrRT, intrST, intrTR, intrTS):
    ...
    for i in range(n-1):
        ...
        dt = intrTR * currT * currR + intrTS * currR * currS
        currR = currR + dr
        currS = currS + ds
        currT = currT + dt
        if currR < 0: currR = 0
...
def one_step_error_t(params, n, grow_vals, init_pop_vals, intr_vals, true_exp_params):
    intrRT = params[0]; intrST = params[1]; intrTR = params[2]; intrTS = params[3]
